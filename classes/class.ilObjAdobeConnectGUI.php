...
//		            case "editContents":
    ...
    public function setTabs()
    {
        ...
        if ($this->access->checkAccess("read", "", $this->object->getRefId()) || $is_member) {
            $this->tabs->addTab("contents", $this->txt("adobe_meeting_room"),
        ...
        if ($xavc_access || $is_admin || ilObject::_lookupOwner(ilObject::_lookupObjectId($ref_id)) == $ilUser->getId()) {
            $this->tabs->addTab("participants", $this->txt("participants"),
...
//			break;
            case 'participants':
                $xavc_access = ilXAVCPermissions::hasAccess($ilUser->getId(), $this->object->getRefId(),
                    AdobeConnectPermissions::PERM_EDIT_PARTICIPANTS);
                $is_owner = $ilUser->getId() == $this->object->getOwner();
                
                if ($is_owner || $xavc_access) {
                    $this->tabs->addSubTab("editParticipants", $lng->txt("crs_member_administration"),
                        $this->ctrl->getLinkTarget($this, 'editParticipants'));
                    
                    if (!ilAdobeConnectServer::getSetting('allow_crs_grp_trigger') && count($this->object->getParticipantsObject()->getParticipants()) > 0) {
                        $this->tabs->addSubTab("addCrsGrpMembers", $this->txt("add_crs_grp_members"),
                            $this->ctrl->getLinkTarget($this, 'addCrsGrpMembers'));
                    }
                }
                $this->tabs->addSubTab("showMembersGallery", $this->pluginObj->txt('members_gallery'),
                    $this->ctrl->getLinkTarget($this, 'showMembersGallery'));
                break;
        }
    }
    ...
    public function initPropertiesForm()
    {
        ...
        $opt_private = new ilRadioOption($this->pluginObj->txt('private_room'),
            ilObjAdobeConnect::ACCESS_LEVEL_PRIVATE);
        $opt_protected = new ilRadioOption($this->pluginObj->txt('protected_room'),
        ...
        $radio_time_type = new ilRadioGroupInputGUI($this->pluginObj->txt('time_type_selection'),
            'time_type_selection');
    ...
    public function updateProperties()
    {
        ...
        if ($this->form->getInput('time_type_selection') == 'permanent_room' && ilAdobeConnectServer::getSetting('enable_perm_room',
                '1')) {
            $durationValid = true;
        } else {
        ...
        if ($formValid && $durationValid) {
            ...
            $enable_perm_room = (ilAdobeConnectServer::getSetting('enable_perm_room',
                    '1') && $this->form->getInput('time_type_selection') == 'permanent_room') ? true : false;
            ...
            if (in_array($this->form->getInput('access_level'), array(
                ilObjAdobeConnect::ACCESS_LEVEL_PRIVATE,
                ilObjAdobeConnect::ACCESS_LEVEL_PROTECTED,
            ...
            if (!$time_mismatch || ($this->form->getInput('time_type_selection') == 'permanent_room' && ilAdobeConnectServer::getSetting('enable_perm_room',
                        '1'))) {
    ...
    private function doProvideAccessLink(ilDateTime $now = null)
    {
        ...
        $datetime_before = new ilDateTime($this->object->getStartDate()->getUnixtime() - $instance->getBufferBefore(),
            IL_CAL_UNIX);
        $datetime_after = new ilDateTime($this->object->getEndDate()->getUnixtime() + $instance->getBufferAfter(),
            IL_CAL_UNIX);
        if (!isset($now)) {
        ...
        if ($this->object->getStartDate() != null) {
            ...
            if ($this->doProvideAccessLink() && $this->object->isParticipant($xavc_login)) {
                if (!$quota->mayStartScheduledMeeting($this->object->getScoId())) {
                    ...
                    $href = '<a href="' . $this->ctrl->getLinkTarget($this,
                            'performSso') . '" target="_blank" >' . $presentation_url . $this->object->getURL() . '</a>';
                    $link->setHtml($href);
                }
            } else {
        ...
        if (null !== $this->object->getStartDate()) {
            ...
            if ($settings->getAuthMode() == ilAdobeConnectServer::AUTH_MODE_SWITCHAAI) {
                ...
                if (($this->object->getPermanentRoom() == 1 || $this->doProvideAccessLink())) {
                    ...
                    $sso_tpl = new ilTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.perform_sso.html",
                        true, true);
            ...
            foreach ($contents as $content) {
                ...
                if ($has_access && $content_type == $by_type) {
                    ...
                    if ($content_type == 'content') {
                        ...
                        $action->addItem($this->lng->txt('delete'), '',
...
//					}
    ...
    public function addCrsGrpMembers()
    {
        ...
        $my_tpl = new ilTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.meeting_participant_table.html",
            true, true);
        ...
        $tbl_admin->setRowTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.meeting_participant_row.html",
            false);
        ...
        if (isset($_POST['usr_id'])) {
            foreach ($_POST['usr_id'] as $selected_user) {
                if (array_key_exists($selected_user, $_POST['xavc_status'])) {
                    ...
                    $this->object->updateParticipant(ilXAVCMembers::_lookupXAVCLogin($selected_user),
                        $memberObj->getStatus());
                }
                
                if (isset($_POST['roles'][$selected_user])) {
                    $deassign_roles = array_diff($local_roles, $_POST['roles'][$selected_user]);
                    if ($deassign_roles) {
                    ...
                    foreach ($_POST['roles'][$selected_user] as $role_id) {
    ...
    public function requestAdobeConnectContent()
    {
        ...
        $sso_tpl = new ilTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.perform_sso.html", true,
            true);
        $sso_tpl->setVariable('SPINNER_SRC', $this->pluginObj->getDirectory() . '/templates/js/spin.js');
        $sso_tpl->setVariable('TITLE_PREFIX', $title_prefix);
        $sso_tpl->setVariable('LOGOUT_URL', str_replace(['http://', 'https://'], '//', $logout_url));
    ...
    public function addParticipant($a_user_id)
    {
        ...
        if (!$is_member) {
            ...
            if ($is_member) {
                ...
                $this->object->updateParticipant(ilXAVCMembers::_lookupXAVCLogin($a_user_id),
                    ilXAVCMembers::_lookupStatus($a_user_id, $this->object->getRefId()));
    ...
    public function updateContent()
    {
        ...
        if ($this->cform->checkInput()) {
            ...
            try {
                ...
                if ($fdata['name'] != '') {
                    $this->object->uploadFile($this->object->uploadContent((int) $_GET['content_id']),
                        $target . '/' . $fdata['name']);
...
//        login to ac-server
        ...
        if ($template_settings['reuse_existing_rooms']['hide'] == '0') {
            ...
            if ($free_scos && $radio_existing) {
                ...
                $contact_info_3 = new ilTextAreaInputGUI($this->pluginObj->txt("contact_information"),
                    "contact_info_3");
            ...
            if (ilAdobeConnectServer::getSetting('default_perm_room') && ilAdobeConnectServer::getSetting('enable_perm_room',
                    '1')) {
        ...
        if (ilAdobeConnectServer::getSetting('use_meeting_template')) {
            $use_meeting_template = new ilCheckboxInputGUI($this->pluginObj->txt('use_meeting_template'),
    ...
    public function getFreeMeetingSlotTable($meetings)
    {
        ...
        foreach ($meetings as $m) {
            ...
            if ($day0s == $day1s) {
                ...
                $h0 = date('H', $m->start_date - $buffer_before) * 60 + floor(date('i',
                            $m->start_date - $buffer_before) / 15) * 15;
                $h1 = date('H', $m->end_date + $buffer_after) * 60 + floor(date('i',
                            $m->end_date + $buffer_after) / 15) * 15;
        ...
        if ($form->checkInput()) {
            ...
            } else // create new object
            {
                ...
                if ($template_settings['duration']['hide'] == '1') {
                    ...
                    if ($form->getInput('time_type_selection') == 'permanent_room' && ilAdobeConnectServer::getSetting('enable_perm_room',
                            '1')) {
                        $duration['hh'] = 2;
                        $duration['mm'] = 0;
                    } else {
                ...
                if ($template_settings['start_date']['hide'] == '1') {
                    ...
                    if ($durationValid) {
                        ...
                        $time_mismatch = false;
                        if (ilDateTime::_before($newStartDate,
                                $minTime) && $form->getInput('time_type_selection') != 'permanent_room') {
                            ilUtil::sendFailure(sprintf($this->pluginObj->txt('xavc_lead_time_mismatch_create'),
                                ilDatePresentation::formatDate($minTime)), true);
                            $time_mismatch = true;
                        }
                    }
                }
    ...
    private function showCreationForm(ilPropertyFormGUI $form)
    {
        ...
        foreach ($ranges as $r) {
            ...
            $data[] = array(
                ...
                'sched_size' => str_pad($size_hours, 2, '0', STR_PAD_LEFT) . ':' . str_pad($size_minutes, 2, '0',
                        STR_PAD_LEFT),
            );
        }
...
//		$tgui->setRowTemplate('tpl.schedule_row.html', 'Customizing/global/plugins/Services/Repository/RepositoryObject/AdobeConnect');
    ...
    public function editParticipants()
    {
        ...
        $has_access = ilXAVCPermissions::hasAccess($ilUser->getId(), $this->object->getRefId(),
    ...
    public function getPerformTriggerHtml()
    {
        ...
        $trigger_tpl = new ilTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.perform_trigger.html",
            true, true);
...
// show link
        if (($this->object->getPermanentRoom() == 1 || $this->doProvideAccessLink())
            ...
            if (!$quota->mayStartScheduledMeeting($this->object->getScoId())) {
                ...
                $button_target = ILIAS_HTTP_PATH . "/" . $this->ctrl->getLinkTarget($this, 'performSso', '', false,
                        false);
                $button_tpl = new ilTemplate($this->pluginObj->getDirectory() . "/templates/default/tpl.bigbutton.html",
                    true, true);
    ...
    public function showContent()
    {
        ...
        if (($this->access->checkAccess("write", "",
                    $this->object->getRefId()) || $is_member || $is_admin) && !$show_only_roomlink) {
            $presentation_url = $settings->getPresentationUrl();
            ...
            if ($this->object->getStartDate() != null) {
                ...
                //show contents
                if (
                    ilXAVCPermissions::hasAccess($ilUser->getId(), $this->object->getRefId(),
                    ...
                    $has_access = false;
                    if (
                        ilXAVCPermissions::hasAccess($ilUser->getId(), $this->ref_id,
