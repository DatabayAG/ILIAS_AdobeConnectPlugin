...
//		            case "editContents":
    ...
    public function assignRolesAfterCreate()
    {
        ...
        if (ilAdobeConnectServer::getSetting('add_to_desktop') == 1) {
            ilObjAdobeConnect::addToFavourites($ilUser->getId(), $this->object->getRefId());
        }
...
//			break;
            case 'participants':
                ...
                if ($is_owner || $xavc_access) {
                    $this->tabs->addSubTab(
                        "editParticipants",
                        $lng->txt("crs_member_administration"),
                        $this->ctrl->getLinkTarget($this, 'editParticipants')
                    );
    ...
    public function updateProperties()
    {
        ...
        if ($formValid && $durationValid) {
            ...
            if (!$time_mismatch || ($this->form->getInput('time_type_selection') == 'permanent_room' && ilAdobeConnectServer::getSetting(
                'enable_perm_room',
                '1'
            ))) {
 ...
 */
    ...
    public function addParticipant($a_user_id)
    {
        ...
        // add to desktop
        if (ilAdobeConnectServer::getSetting('add_to_desktop') == 1) {
            ilObjAdobeConnect::addToFavourites($a_user_id, $this->object->getRefId());
        }
        ...
        if (!$is_member) {
            ...
            if ($is_member) {
                ...
                $this->object->updateParticipant(
                    ilXAVCMembers::_lookupXAVCLogin($a_user_id),
            ...
            try {
                ...
                if ($fdata['name'] != '') {
                    $this->object->uploadFile(
                        $this->object->uploadContent((int) $_GET['content_id']),
                        $target . '/' . $fdata['name']
                    );
...
//        login to ac-server
    ...
    public function getFreeMeetingSlotTable($meetings)
    {
        ...
        foreach ($meetings as $m) {
            ...
            if ($day0s == $day1s) {
                ...
                $h0 = date('H', $m->start_date - $buffer_before) * 60 + floor(date(
                    'i',
                    $m->start_date - $buffer_before
                ) / 15) * 15;
                $h1 = date('H', $m->end_date + $buffer_after) * 60 + floor(date(
                    'i',
    ...
    public function editParticipants()
    {
        ...
        if ((count($this->object->getParticipantsObject()->getParticipants()) == 0 && $has_access) || $is_owner) {
            ...
            // search button
            $ilToolbar->addButton(
                $this->lng->txt("crs_search_members"),
    ...
    public function showContent()
    {
        ...
        if (($this->access->checkAccess(
            ...
            "",
            $this->object->getRefId()
        ) || $is_member || $is_admin) && !$show_only_roomlink) {
            $presentation_url = $settings->getPresentationUrl();
